...
状态的转移矩阵为$A=[a_{ij}]_{N\times N}$，其中$a_{ij}=P(i_{t+1}=q_j|i_t=q_i),i=1,2,...,N,j=1,2,...,N$表示在时刻$t$下不可观测的状态序列从$q_i$转移到$q_j$的概率。

状态观测转移的观测概率矩阵为$B=[b_j(k)]_{N\times M},j=1,2,...,N,k=1,2,...,M$，其中$b_j(k)=P(o_t=v_k|i_t=q_j)$表示在时刻$t$下不可观测的状态$q_j$转化为可观测的观测$v_k$的概率。

状态的初始概率向量为$\pi=(\pi_i)$，其中$\pi_i=P(i_1=q_i),i=1,2,...,N$表示$t_1$时刻下处于状态$q_i$的概率。
...
近似算法在每个时刻$t$分别选择该时刻最有可能出现的状态$i_t^*$，从而得到一个状态序列$I^*=(i_1^*,i_2^*,...,i_T^*)$。

由于$\gamma_t(i)=\frac{P(i_t=q_i,O|\lambda)}{P(O|\lambda)}$，因此易于发现：
